...
fn web_body_limit_bytes() -> usize {
    ...
    let mb = read_env_with_legacy("TASK_MAX_UPLOAD_MB", "TASK_MAX_UPLOAD_MB")
        .ok()
...
async fn main() {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "task_web=info,tower_http=info".parse().unwrap()),
        )
        .init();
    ...
    // Data directory
    let data_dir =
        read_env_with_legacy("TASK_DATA_DIR", "TASK_DATA_DIR").map(std::path::PathBuf::from).unwrap_or_else(|_| {
            let home = std::env::var("HOME").unwrap_or_else(|_| ".".to_string());
            let task_dir = std::path::PathBuf::from(&home).join(".task-web");
            if task_dir.exists() {
                task_dir
            } else {
                let legacy_dir = std::path::PathBuf::from(&home).join(".task-web");
    ...
    let password_hash = if let Ok(pw) = read_env_with_legacy("TASK_PASSWORD", "TASK_PASSWORD") {
    ...
    // Static file serving
    if let Ok(static_dir) = read_env_with_legacy("TASK_STATIC_DIR", "TASK_STATIC_DIR") {
    ...
    // Bind address
    let port: u16 = read_env_with_legacy("TASK_PORT", "TASK_PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(4224);
    let addr = SocketAddr::from(([0, 0, 0, 0], port));

    tracing::info!("TASK web server starting on http://{}", addr);
    if read_env_with_legacy("TASK_PASSWORD", "TASK_PASSWORD").is_ok() {
