...
pub async fn save_schema_cache(
    ...
    state.app.storage.save_schema_cache(&body.cache_key, &body.payload).await.map_err(AppError::internal)?;
    Ok(Json(()))
}
...
pub async fn load_schema_cache(
    ...
    let payload = state.app.storage.load_schema_cache(&query.cache_key).await.map_err(AppError::internal)?;
...
pub async fn delete_schema_cache_prefix(
    ...
    state.app.storage.delete_schema_cache_prefix(&query.prefix).await.map_err(AppError::internal)?;
    Ok(Json(()))
}
