...
pub async fn save_sidebar_layout(
    ...
    state.app.storage.save_sidebar_layout(&body.layout).await.map_err(AppError::internal)?;
    Ok(Json(()))
}

pub async fn load_sidebar_layout(State(state): State<Arc<WebState>>) -> Result<Json<serde_json::Value>, AppError> {
    let layout = state.app.storage.load_sidebar_layout().await.map_err(AppError::internal)?;
