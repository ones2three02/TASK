...
pub async fn stop_driver_runtime(
    ...
    task_core::driver_runtime::stop_driver_runtime(&state.app, &req.runtime_id).await.map_err(AppError)?;
...
pub async fn restart_driver_runtime(
    ...
    task_core::driver_runtime::restart_driver_runtime(&state.app, &req.runtime_id).await.map_err(AppError)?;
    Ok(Json(serde_json::json!({ "ok": true })))
}
