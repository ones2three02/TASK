...
pub async fn get_driver_runtime_summary(
    ...
    Ok(Json(task_core::driver_runtime::collect_driver_runtime_summary(&state.app).await))
}
...
pub async fn stop_driver_runtime(
    ...
    task_core::driver_runtime::stop_driver_runtime(&state.app, &req.runtime_id).await.map_err(AppError::internal)?;
...
pub async fn restart_driver_runtime(
    ...
    task_core::driver_runtime::restart_driver_runtime(&state.app, &req.runtime_id).await.map_err(AppError::internal)?;
    Ok(Json(serde_json::json!({ "ok": true })))
}
...
pub async fn upgrade_all_agents(State(state): State<Arc<WebState>>) -> Result<Json<serde_json::Value>, AppError> {
    let registry = fetch_registry().await.map_err(AppError::internal)?;
    ...
    ensure_no_agent_update_blockers(&state.app, &updatable).await.map_err(AppError::internal)?;
    let tx = progress_sender(&state, "global").await;
    let result = upgrade_all_agent_drivers(&state.app.agent_manager, |event| send_progress_event(&tx, event))
        .await
        .map_err(AppError::internal)?;
    Ok(Json(serde_json::to_value(result).map_err(|err| AppError::internal(err.to_string()))?))
}
...
pub async fn uninstall_agent(
    ...
    uninstall_agent_driver(&state.app.agent_manager, &req.db_type).await.map_err(AppError::internal)?;
    Ok(Json(serde_json::json!({ "ok": true })))
}
...
pub async fn set_agent_java_runtime_config(
    ...
    if config.mode == JavaRuntimeMode::Custom || config.mode == JavaRuntimeMode::System {
        ...
        let resolved = am.resolve_java_runtime(&candidate_state, DEFAULT_JRE_KEY).map_err(AppError::internal)?;
    ...
    am.save_state(&local_state).map_err(AppError::internal)?;
...
pub async fn import_agents_from_zip(
    ...
    std::fs::create_dir_all(&tmp_dir).map_err(|err| AppError::internal(err.to_string()))?;

    if let Some(field) = multipart.next_field().await.map_err(|err| AppError::internal(err.to_string()))? {
        let file_name = field.file_name().unwrap_or("offline-drivers.zip").to_string();
        if !file_name.to_ascii_lowercase().ends_with(".zip") {
            return Err(AppError::internal("Offline driver package must be a .zip file".to_string()));
        }

        let zip_path = tmp_dir.join(format!("agent-offline-{}.zip", uuid::Uuid::new_v4()));
        let mut upload = tokio::fs::File::create(&zip_path).await.map_err(|err| AppError::internal(err.to_string()))?;
        let mut field = field;
        while let Some(chunk) = field.chunk().await.map_err(|err| AppError::internal(err.to_string()))? {
            upload.write_all(&chunk).await.map_err(|err| AppError::internal(err.to_string()))?;
        }
        upload.flush().await.map_err(|err| AppError::internal(err.to_string()))?;
        drop(upload);
...
pub async fn import_agent_jar(
    ...
    while let Ok(Some(field)) = multipart.next_field().await {
        ...
        if name == "dbType" {
            db_type = Some(field.text().await.map_err(|e| AppError::internal(e.to_string()))?);
        } else if name == "file" {
            jar_name = field.file_name().unwrap_or("driver.jar").to_string();
            if !jar_name.to_lowercase().ends_with(".jar") {
                return Err(AppError::internal("Only .jar files can be imported".to_string()));
            }
            jar_data = Some(field.bytes().await.map_err(|e| AppError::internal(e.to_string()))?.to_vec());
        }
    }

    let db_type = db_type.ok_or_else(|| AppError::internal("Missing dbType field".to_string()))?;
    let data = jar_data.ok_or_else(|| AppError::internal("No file uploaded".to_string()))?;
    ...
    task_core::agent_service::import_agent_jar(&state.app.agent_manager, &db_type, &tmp_path)
...
pub async fn uninstall_jre(
    ...
    uninstall_agent_jre(&state.app.agent_manager, key).await.map_err(AppError::internal)?;
    Ok(Json(serde_json::json!({ "ok": true })))
}
...
async fn ensure_no_agent_update_blockers(
    state: &task_core::connection::AppState,
    db_types: &[String],
) -> Result<(), String> {
    ...
    let mut blockers = state
        ...
        .map(|key| task_core::agent_catalog::label_for_key(&key).unwrap_or(&key).to_string())
        .collect::<Vec<_>>();
    blockers.sort();
