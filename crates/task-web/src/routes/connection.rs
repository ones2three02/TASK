...
mod tests {
    ...
    use std::sync::Arc;
    use task_core::connection::{AppState, PoolKind};
    ...
    async fn test_web_state() -> (Arc<WebState>, std::path::PathBuf) {
        let dir = std::env::temp_dir().join(format!("task-web-test-{}", uuid::Uuid::new_v4()));
    ...
    async fn disconnect_db_keeps_connections_with_similar_prefixes() {
        ...
        let conn_pool = task_core::db::sqlite::connect_path(&conn_path.to_string_lossy()).await.unwrap();
        let conn2_pool = task_core::db::sqlite::connect_path(&conn2_path.to_string_lossy()).await.unwrap();
