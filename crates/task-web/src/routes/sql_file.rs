...
use task_core::sql;
use task_core::sql::{SqlFileProgress, SqlFileRequest, SqlFileStatus};
use task_core::sql_file_import::{
...
pub async fn preview_sql_file(
    ...
    if let Some(field) = multipart.next_field().await.map_err(|e| AppError::internal(e.to_string()))? {
        ...
        let content = sql::decode_sql_file_bytes(&data).map_err(AppError::internal)?;
...
pub async fn execute_sql_file(
...

    {
        ...
        if executions.contains_key(&execution_id) {
            return Err(AppError::internal(format!("SQL file execution '{execution_id}' already exists")));
        }
...
fn safe_uploaded_sql_path(tmp_dir: &Path, file_name: &str) -> Result<PathBuf, AppError> {
    ...
    if base_name.is_empty() || base_name == "." || base_name == ".." {
        return Err(AppError::internal("Invalid SQL file name".to_string()));
    }
...
fn validated_uploaded_sql_path(data_dir: &Path, file_path: &str) -> Result<PathBuf, AppError> {
    ...
    if !path.is_absolute() {
        return Err(AppError::internal("File path must be absolute".to_string()));
    }

    let tmp_dir = data_dir.join("tmp").canonicalize().map_err(|e| AppError::internal(e.to_string()))?;
    let canonical_path = path.canonicalize().map_err(|e| AppError::internal(e.to_string()))?;
    if !canonical_path.starts_with(&tmp_dir) {
        return Err(AppError::internal("File path must be inside the uploaded SQL directory".to_string()));
    }
    Ok(canonical_path)
}
...
pub async fn sql_file_progress(
    ...
    let tx = channels.get(&execution_id).ok_or_else(|| AppError::internal("Execution not found".to_string()))?;
    let rx = tx.subscribe();
    drop(channels);
...
mod tests {
    ...
    fn uploaded_sql_path_uses_only_the_file_name() {
        let data_dir = std::env::temp_dir().join(format!("task-web-sql-file-test-{}", uuid::Uuid::new_v4()));
