...
use task_core::table_export::{self, ExportStatus, TableExportProgress, TableExportRequest};
...
pub async fn start_table_export(
    ...
    let ext = match req.format.as_str() {
        ...
        "markdown" | "md" => "md",
        "sql" => "sql",
        _ => return Err(AppError::internal(format!("Unsupported export format: {}", req.format))),
    };
    ...
    tokio::spawn(async move {
        ...
        task_core::database_export::clear_export_cancelled(&req.export_id).await;
...
pub async fn cancel_table_export(
    ...
    task_core::database_export::set_export_cancelled(&req.export_id).await;
...
pub async fn table_export_download(
    ...
    let (file_path, format) = state
        ...
        .await
        .remove(&export_id)
        .ok_or_else(|| AppError::internal("Export file not found".to_string()))?;

    let data = tokio::fs::read(&file_path).await.map_err(|e| AppError::internal(e.to_string()))?;
    // Clean up temp file
    ...
    let (content_type, file_ext) = match format.as_str() {
        ...
        _ => return Err(AppError::internal(format!("Unknown format: {format}"))),
    };
