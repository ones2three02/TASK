...
pub async fn prepare_data_compare(
    ...
    task_core::data_compare::prepare_data_compare(options).map(Json).map_err(AppError)
}
...
pub async fn prepare_data_compare_from_tables(
    ...
    task_core::data_compare::prepare_data_compare_from_tables(&state.app, options).await.map(Json).map_err(AppError)
}
...
pub async fn prepare_data_compare_missing_target(
    ...
    task_core::data_compare::prepare_data_compare_missing_target(&state.app, options).await.map(Json).map_err(AppError)
}
