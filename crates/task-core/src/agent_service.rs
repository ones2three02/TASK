...
const REGISTRY_PATH: &str = "https://github.com/ones2three02/TASK/releases/latest/download/agent-registry.json";
...
pub fn local_agent_jar_candidates(db_type: &str) -> Vec<PathBuf> {
    let jar_name = format!("task-agent-{db_type}.jar");
    let relative_driver = PathBuf::from("..")
        .join("task-agents")
        .join("drivers")
        .join(db_type)
        .join("build")
        ...
        .join(&jar_name);
    let nested_driver =
        PathBuf::from("task-agents").join("drivers").join(db_type).join("build").join("libs").join(&jar_name);
    let relative_legacy =
        PathBuf::from("..").join("task-agents").join(db_type).join("build").join("libs").join(&jar_name);
    let nested_legacy = PathBuf::from("task-agents").join(db_type).join("build").join("libs").join(&jar_name);
...
pub async fn fetch_registry() -> Result<AgentRegistry, String> {
    ...
    let resp = crate::race_download(&client, REGISTRY_PATH, REGISTRY_R2_PATH, "task-agent-manager")
        .await
...
async fn download_with_progress(
    ...
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|err| format!("Failed to create HTTP client: {err}"))?;
    let mut resp = crate::race_download(&client, url, r2_path, "task-agent-manager")
        .await
...
pub fn ensure_driver_app_version(
    ...
    Err(format!(
        "{db_type} driver {} requires TASK {} or newer. Current TASK version is {}.",
...
fn extract_db_type_from_filename(name: &str) -> Option<String> {
    let filename = name.rsplit('/').next()?;
    let rest = filename.strip_prefix("task-agent-")?;
