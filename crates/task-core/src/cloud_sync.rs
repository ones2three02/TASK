...
const DEFAULT_REMOTE_PATH: &str = "TASK/sync/snapshot.json";
...
mod tests {
    ...
    fn normalizes_empty_remote_path_to_default() {
        assert_eq!(normalized_remote_path(None), "TASK/sync/snapshot.json");
        assert_eq!(normalized_remote_path(Some("")), "TASK/sync/snapshot.json");
    ...
    fn returns_parent_collection_paths_from_leaf() {
        assert_eq!(
            parent_collection_paths("task/sync/snapshot.json"),
            vec!["task".to_string(), "task/sync".to_string()]
        );
    }
