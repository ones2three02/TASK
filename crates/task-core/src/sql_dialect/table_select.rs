...
use super::types::{
    TableDataSelectSqlOptions, TableSelectSqlOptions, TASK_NEO4J_ELEMENT_ID_COLUMN, TASK_ROWID_COLUMN,
...
pub fn build_table_data_select_sql(options: TableDataSelectSqlOptions) -> String {
    ...
    let default_order_alias = if database_type == Some(DatabaseType::Jdbc) { Some("task_t") } else { row_id_alias };
    ...
    let select_columns = if options.include_row_id && database_type == Some(DatabaseType::Oracle) {
        format!("ROWIDTOCHAR(t.ROWID) AS \"{TASK_ROWID_COLUMN}\", t.*")
    } else {
...
pub(super) fn is_oracle_row_id(database_type: Option<DatabaseType>, name: &str) -> bool {
    database_type == Some(DatabaseType::Oracle) && name.eq_ignore_ascii_case(TASK_ROWID_COLUMN)
}

pub(super) fn is_tdengine_tbname(database_type: Option<DatabaseType>, name: &str) -> bool {
    database_type == Some(DatabaseType::Tdengine) && name.eq_ignore_ascii_case(TASK_TDENGINE_TBNAME_COLUMN)
}
...
pub(super) fn quote_order_identifier(
    ...
    if is_tdengine_tbname(database_type, name) {
        return TASK_TDENGINE_TBNAME_COLUMN.to_string();
    }
...
pub(super) fn build_select_columns(database_type: Option<DatabaseType>, columns: &[String]) -> String {
    ...
    if database_type == Some(DatabaseType::Tdengine) {
        ...
        if !columns.iter().any(|column| column.eq_ignore_ascii_case(TASK_TDENGINE_TBNAME_COLUMN)) {
            tdengine_columns.push(TASK_TDENGINE_TBNAME_COLUMN.to_string());
        }
        ...
        return tdengine_columns
            ...
            .map(|column| {
                if is_tdengine_tbname(database_type, column) {
                    TASK_TDENGINE_TBNAME_COLUMN.to_string()
                } else {
...
pub(super) fn build_sqlserver_table_select_sql(
    ...
    let page_alias = quote_table_identifier(Some(DatabaseType::SqlServer), "task_page");
    let row_number_alias = quote_table_identifier(Some(DatabaseType::SqlServer), "__task_row_num");
...
pub(super) fn build_db2_table_select_page_sql(
    ...
    let inner_columns = if columns.is_empty() {
        ...
        columns
            .iter()
            .map(|column| format!("task_t.{}", quote_table_identifier(Some(DatabaseType::Db2), column)))
            .collect::<Vec<_>>()
            .join(", ")
    };
    let order = order_by.map(|order_by| format!("ORDER BY {order_by}")).unwrap_or_default();
    let row_number = quote_table_identifier(Some(DatabaseType::Db2), "__task_row_num");
    let end = offset + limit;

    format!(
        "SELECT {columns_sql} FROM (SELECT {inner_columns}, ROW_NUMBER() OVER ({order}) AS {row_number} FROM {table} task_t{where_clause}) task_page WHERE {row_number} > {offset} AND {row_number} <= {end} ORDER BY {row_number}"
    )
}
...
pub(super) fn build_neo4j_table_select_sql(options: &TableDataSelectSqlOptions, limit: usize) -> String {
    ...
    let returns = format!(
        ...
        quote_table_identifier(Some(DatabaseType::Neo4j), TASK_NEO4J_ELEMENT_ID_COLUMN)
    );
