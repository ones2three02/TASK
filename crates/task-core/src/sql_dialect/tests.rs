...
fn qualifies_schema_only_for_schema_aware_databases() {
    ...
    assert_eq!(qualified_table_name(Some(DatabaseType::Databend), Some("task_test"), "users"), "`task_test`.`users`");
...
fn builds_select_sql_with_limit_syntax_for_database_type() {
    ...
    assert_eq!(
        build_table_select_sql(TableSelectSqlOptions {
            ...
            schema: Some("task_test"),
            table_name: "jdbc_probe",
            columns: &[],
            order_columns: &[],
            limit: 500,
        }),
        "SELECT * FROM `task_test`.`jdbc_probe` LIMIT 500;"
    );
    assert_eq!(
        build_table_select_sql(TableSelectSqlOptions {
...
fn builds_table_data_where_and_schema_queries() {
    ...
    assert_eq!(
            ...
            "SELECT \"ID\", \"AMOUNT\" FROM (SELECT task_t.\"ID\", task_t.\"AMOUNT\", ROW_NUMBER() OVER (ORDER BY \"ID\" ASC) AS \"__task_row_num\" FROM \"DB2INST1\".\"ORDERS\" task_t WHERE (amount > 10)) task_page WHERE \"__task_row_num\" > 100 AND \"__task_row_num\" <= 150 ORDER BY \"__task_row_num\""
        );
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            database_type: Some(DatabaseType::Iris),
...
fn builds_sqlserver_table_data_pages() {
    ...
    assert_eq!(
            ...
            "WITH [task_page] AS (SELECT [order_id], [customer], ROW_NUMBER() OVER (ORDER BY [order_id] ASC) AS [__task_row_num] FROM [sales].[orders]) SELECT [order_id], [customer] FROM [task_page] WHERE [__task_row_num] > 100 AND [__task_row_num] <= 150 ORDER BY [__task_row_num]"
        );
}
...
fn builds_oracle_and_neo4j_table_data_queries() {
    assert_eq!(
            build_table_data_select_sql(TableDataSelectSqlOptions {
                ...
                schema: Some("TASKTEST".to_string()),
                table_name: "TASK_LOAD_TABLE_006".to_string(),
                primary_keys: vec![TASK_ROWID_COLUMN.to_string()],
                columns: Vec::new(),
                fallback_order_columns: Vec::new(),
                order_by: None,
                limit: Some(100),
            ...
            "SELECT ROWIDTOCHAR(t.ROWID) AS \"__TASK_ROWID\", t.* FROM \"TASKTEST\".\"TASK_LOAD_TABLE_006\" t ORDER BY t.ROWID ASC"
        );
    assert_eq!(
            build_table_data_select_sql(TableDataSelectSqlOptions {
            ...
            "MATCH (n:`Employee`) RETURN elementId(n) AS `__TASK_ELEMENT_ID`, n.`id` AS `id`, n.`first name` AS `first name`, n.`role` AS `role` ORDER BY n.`id` ASC LIMIT 100;"
        );
}
