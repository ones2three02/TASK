...
const JDBC_PLUGIN_DOWNLOAD_URL: &str =
    "https://github.com/ones2three02/TASK/releases/latest/download/task-jdbc-plugin-latest.zip";
const JDBC_PLUGIN_R2_PATH: &str = "releases/latest/task-jdbc-plugin-latest.zip";
...
fn jdbc_maven_resolver_executable(plugin_dir: &Path) -> PathBuf {
    #[cfg(windows)]
    {
        plugin_dir.join("bin").join("task-maven-resolver.bat")
    }
    #[cfg(not(windows))]
    {
        plugin_dir.join("bin").join("task-maven-resolver")
    }
}
...
async fn download_jdbc_plugin_zip_with_progress(progress: &impl Fn(AgentProgressEvent)) -> Result<Vec<u8>, String> {
    ...
    let mut resp =
        crate::race_download(&client, JDBC_PLUGIN_DOWNLOAD_URL, JDBC_PLUGIN_R2_PATH, "task-jdbc-plugin-installer")
            .await
...
fn install_jdbc_plugin_zip(bytes: &[u8], plugin_dir: &Path) -> Result<(), String> {
    ...
    if manifest.protocol_version != SUPPORTED_PLUGIN_PROTOCOL_VERSION {
        let _ = std::fs::remove_dir_all(&temp_dir);
        return Err(format!(
            "Downloaded JDBC plugin uses protocol version {}, but this TASK build supports protocol version {}",
    ...
    #[cfg(unix)]
    {
        ...
        for executable in ["task-jdbc-plugin", "task-maven-resolver"] {
...
mod tests {
    ...
    fn maven_bundle_install_lists_nested_jars() {
        let root = std::env::temp_dir().join(format!("task-jdbc-maven-test-{}", uuid::Uuid::new_v4()));
