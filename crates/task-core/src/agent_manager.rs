...
mod tests {
    ...
    fn test_manager(name: &str) -> AgentManager {
        let dir = std::env::temp_dir().join(format!("task-agent-manager-{name}-{}", uuid::Uuid::new_v4()));
    ...
    fn stores_configured_app_version_for_agent_handshake() {
        let dir = std::env::temp_dir().join(format!("task-agent-manager-version-{}", uuid::Uuid::new_v4()));
...
impl AgentManager {
    pub fn new() -> Self {
        ...
        Self::new_with_base_dir(PathBuf::from(home).join(".task").join("agents"))
    }
