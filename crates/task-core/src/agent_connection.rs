...
mod tests {
    ...
    fn trino_agent_custom_jdbc_url_rewrites_forwarded_host_and_preserves_query_ssl() {
        ...
        cfg.port = 8443;
        cfg.ssl = true;
        cfg.connection_string = Some("jdbc:trino://trino.internal:8443/hive?SSL=true&source=task".to_string());
        cfg.url_params = Some("SSLVerification=NONE".to_string());
        ...
        assert_eq!(
            params["connection_string"],
            "jdbc:trino://127.0.0.1:15443/hive?SSL=true&source=task&SSLVerification=NONE"
        );
    }
}
