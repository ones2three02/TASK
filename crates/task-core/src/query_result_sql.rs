...
pub fn build_count_query_sql(options: CountQuerySqlOptions) -> QuerySqlBuildResult {
    ...
    let alias = quote_table_identifier(options.database_type, "task_count");
    let wrapped_sql = if options.database_type == Some(DatabaseType::SqlServer) {
        sql_server_statement_for_derived_table(&statement)
    } else {
        statement
    };
    ok(format!("SELECT COUNT(*) AS task_total_rows FROM ({wrapped_sql}) {alias};"))
}
...
fn add_sql_server_top(sql: &str, limit: usize) -> String {
    ...
    if sql.len() >= 6 && sql[..6].eq_ignore_ascii_case("SELECT") {
        ...
        format!("SELECT TOP ({limit}) * FROM ({sql}) [task_page]")
    }
}
...
mod tests {
    ...
    fn builds_count_query() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT COUNT(*) AS task_total_rows FROM (WITH cte AS (SELECT 1 AS id) SELECT * FROM cte) `task_count`;"
        );
    }
