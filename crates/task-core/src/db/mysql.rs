...
fn is_task_handled_mysql_url_param(key: &str) -> bool {
    matches!(
        key.to_ascii_lowercase().as_str(),
        "charset"
            | "time_zone"
...
fn mysql_async_url(url: &str) -> Cow<'_, str> {
    ...
    for segment in query.split('&') {
        ...
        if is_task_handled_mysql_url_param(key) {
            changed = true;
            continue;
        }
...
mod tests {
    ...
    fn mysql_tls_url_strips_client_identity_params_before_driver_parse() {
        ...
        let cert = dir.join(format!("task-mysql-client-cert-{}.pem", std::process::id()));
        let key = dir.join(format!("task-mysql-client-key-{}.pem", std::process::id()));
