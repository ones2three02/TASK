...
fn build_spatial_safe_sqlserver_query(sql: &str, columns: &[SqlServerDescribedColumn]) -> Option<String> {
    ...
    let source_alias = quote_sqlserver_identifier("task_spatial_source");
...
mod tests {
    ...
    fn sqlserver_wraps_geometry_columns_as_text() {
        ...
        assert_eq!(
            rewritten,
            "SELECT [landId] = [task_spatial_source].[task_col_1], [polygon] = CASE WHEN [task_spatial_source].[task_col_2] IS NULL THEN NULL ELSE [task_spatial_source].[task_col_2].STAsText() END FROM (SELECT * FROM dbo.tLandPolygon) AS [task_spatial_source]([task_col_1], [task_col_2])"
        );
    }
