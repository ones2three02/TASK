...
mod tests {
    ...
    fn postgres_connection_url_strips_ssl_file_params_before_driver_parse() {
        ...
        let cert = dir.join(format!("task-postgres-cert-{}.pem", std::process::id()));
        let key = dir.join(format!("task-postgres-key-{}.pem", std::process::id()));
        let root = dir.join(format!("task-postgres-root-{}.pem", std::process::id()));
        std::fs::write(&cert, "not a real cert").unwrap();
        std::fs::write(&key, "not a real key").unwrap();
        std::fs::write(&root, "not a real root").unwrap();

        let url = format!(
            "postgres://localhost/db?sslmode=verify-full&sslcert={}&sslkey={}&sslrootcert={}&application_name=task",
            cert.display(),
            key.display(),
            root.display()
        );
        ...
        assert_eq!(parsed.url, "postgres://localhost/db?sslmode=require&application_name=task");
    ...
    fn transaction_recovery_statement_detection_ignores_regular_queries() {
        ...
        assert!(!is_transaction_recovery_statement("UPDATE users SET name = 'task'"));
    }
