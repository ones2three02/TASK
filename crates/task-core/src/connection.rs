...
impl AppState {
    ...
    fn external_driver_runtime_env(&self, driver_id: &str) -> Result<PluginRuntimeEnv, String> {
        ...
        Ok(PluginRuntimeEnv::default().with_var("TASK_JAVA_BIN", java.to_string_lossy().to_string()))
    }
...
fn default_plugin_dir() -> PathBuf {
    default_task_dir().join("plugins")
}

fn default_agent_dir() -> PathBuf {
    default_task_dir().join("agents")
}
...
mod tests {
    ...
    fn validates_h2_database_base_path_when_mv_db_file_exists() {
        let dir = std::env::temp_dir().join(format!("task-h2-test-{}", uuid::Uuid::new_v4()));
    ...
    async fn test_app_state() -> (AppState, std::path::PathBuf) {
        let dir = std::env::temp_dir().join(format!("task-core-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    async fn app_state_uses_explicit_agent_dir() {
        let dir = std::env::temp_dir().join(format!("task-core-agent-dir-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    ...
    async fn jdbc_plugin_env_uses_managed_jre_when_installed() {
        let dir = std::env::temp_dir().join(format!("task-core-jdbc-managed-jre-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new_with_plugin_and_agent_dir_and_app_version(
            storage,
        ...
        assert_eq!(env.get("TASK_JAVA_BIN"), Some(java.to_string_lossy().as_ref()));
        let _ = std::fs::remove_dir_all(dir);
    }

    #[tokio::test]
    async fn jdbc_plugin_env_keeps_wrapper_fallback_when_managed_jre_is_missing() {
        let dir = std::env::temp_dir().join(format!("task-core-jdbc-missing-jre-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new_with_plugin_and_agent_dir_and_app_version(
            storage,
        ...
        assert_eq!(env.get("TASK_JAVA_BIN"), None);
        let _ = std::fs::remove_dir_all(dir);
    }

    #[tokio::test]
    async fn jdbc_plugin_env_uses_custom_java_runtime() {
        let dir = std::env::temp_dir().join(format!("task-core-jdbc-custom-jre-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new_with_plugin_and_agent_dir_and_app_version(
            storage,
    ...
    fn kwdb_endpoint_url_keeps_explicit_sslmode() {
        ...
        config.password = "secret".to_string();
        config.port = 26257;
        config.url_params = Some("sslmode=require&application_name=task".to_string());

        assert_eq!(
            connection_url_for_endpoint(&config, &config.host, config.port),
            "postgres://root:secret@127.0.0.1:26257/defaultdb?sslmode=require&application_name=task"
        );
    }
    ...
    fn gaussdb_endpoint_url_keeps_explicit_sslmode() {
        ...
        assert_eq!(
            connection_url_for_endpoint(&config, &config.host, config.port),
            "postgres://gaussdb:secret@127.0.0.1:3306/postgres?sslmode=require&application_name=task"
        );
    }
    ...
    fn gaussdb_endpoint_url_prepends_default_sslmode_to_custom_params() {
        ...
        config.username = "gaussdb".to_string();
        config.password = "secret".to_string();
        config.url_params = Some("application_name=task".to_string());

        assert_eq!(
            connection_url_for_endpoint(&config, &config.host, config.port),
            "postgres://gaussdb:secret@127.0.0.1:3306/postgres?sslmode=disable&application_name=task"
        );
    }
    ...
    async fn live_gaussdb_native_connection_succeeds() {
        let host = std::env::var("TASK_TEST_GAUSSDB_HOST").expect("TASK_TEST_GAUSSDB_HOST not set");
        let port = std::env::var("TASK_TEST_GAUSSDB_PORT")
            .expect("TASK_TEST_GAUSSDB_PORT not set")
            .parse::<u16>()
            .expect("TASK_TEST_GAUSSDB_PORT should be a u16");
        let username = std::env::var("TASK_TEST_GAUSSDB_USER").expect("TASK_TEST_GAUSSDB_USER not set");
        let password = std::env::var("TASK_TEST_GAUSSDB_PASSWORD").expect("TASK_TEST_GAUSSDB_PASSWORD not set");
        let url_params = std::env::var("TASK_TEST_GAUSSDB_URL_PARAMS").ok();
    ...
    async fn live_opengauss_native_connection_succeeds() {
        let host = std::env::var("TASK_TEST_OPENGAUSS_HOST").expect("TASK_TEST_OPENGAUSS_HOST not set");
        let port = std::env::var("TASK_TEST_OPENGAUSS_PORT")
            .expect("TASK_TEST_OPENGAUSS_PORT not set")
            .parse::<u16>()
            .expect("TASK_TEST_OPENGAUSS_PORT should be a u16");
        let username = std::env::var("TASK_TEST_OPENGAUSS_USER").expect("TASK_TEST_OPENGAUSS_USER not set");
        let password = std::env::var("TASK_TEST_OPENGAUSS_PASSWORD").expect("TASK_TEST_OPENGAUSS_PASSWORD not set");
        let url_params = std::env::var("TASK_TEST_OPENGAUSS_URL_PARAMS").ok();
    ...
    async fn live_kwdb_native_connection_succeeds() {
        let host = std::env::var("TASK_TEST_KWDB_HOST").expect("TASK_TEST_KWDB_HOST not set");
        let port = std::env::var("TASK_TEST_KWDB_PORT")
            .expect("TASK_TEST_KWDB_PORT not set")
            .parse::<u16>()
            .expect("TASK_TEST_KWDB_PORT should be a u16");
        let username = std::env::var("TASK_TEST_KWDB_USER").unwrap_or_else(|_| "root".to_string());
        let password = std::env::var("TASK_TEST_KWDB_PASSWORD").unwrap_or_default();
        let database = std::env::var("TASK_TEST_KWDB_DATABASE").unwrap_or_else(|_| "defaultdb".to_string());
        let url_params = std::env::var("TASK_TEST_KWDB_URL_PARAMS").unwrap_or_else(|_| "sslmode=disable".to_string());
        ...
        let test_schema = "task_kwdb_live";
