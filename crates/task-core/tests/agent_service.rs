...
fn test_manager(name: &str) -> AgentManager {
    let dir = std::env::temp_dir().join(format!("task-agent-service-{name}-{}", uuid::Uuid::new_v4()));
...
fn registry_with_driver(db_type: &str, version: &str, jre: &str) -> AgentRegistry {
    ...
    drivers.insert(
        ...
        DriverInfo {
            ...
            jre: jre.to_string(),
            jar: Some(ArtifactInfo { url: format!("https://example.com/task-agent-{db_type}.jar"), size: 42 }),
...
fn agent_list_marks_installed_driver_update_when_registry_version_differs() {
    ...
    manager
        .save_state(&task_core::agent_manager::AgentState {
            installed_drivers: [(
                "h2".to_string(),
                InstalledDriver {
...
fn local_agent_jar_candidates_include_sibling_build_output() {
    ...
    assert!(candidates.iter().any(|path| path.ends_with("task-agents/tdengine/build/libs/task-agent-tdengine.jar")));
}

#[test]
fn github_agent_asset_urls_map_to_r2_paths_by_category() {
    assert_eq!(
        github_url_to_r2_path("https://github.com/ones2three02/TASK/releases/download/v1/task-jre-21.tar.gz", "jre"),
        "agents/jre/task-jre-21.tar.gz"
    );
    assert_eq!(
        github_url_to_r2_path("https://github.com/ones2three02/TASK/releases/download/v1/task-agent-h2.jar", "driver"),
        "agents/drivers/task-agent-h2.jar"
    );
}

fn test_path(name: &str) -> std::path::PathBuf {
    std::env::temp_dir().join(format!("task-agent-service-{name}-{}", uuid::Uuid::new_v4()))
}
...
fn local_jar_import_updates_driver_state() {
    ...
    let source = test_path("local-import-source").join("task-agent-h2.jar");
...
fn write_offline_driver_zip(path: &std::path::Path, db_type: &str, version: &str) {
    ...
    let registry = serde_json::json!({
        "drivers": {
            db_type: {
                ...
                "jre": DEFAULT_JRE_KEY,
                "jar": { "url": format!("https://example.com/task-agent-{db_type}.jar"), "size": 3 }
            }
        }
    });
    ...
    zip.start_file(format!("drivers/task-agent-{db_type}.jar"), options).unwrap();
