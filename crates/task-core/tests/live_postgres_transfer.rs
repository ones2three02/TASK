...
#[tokio::test]
#[ignore = "requires source/target PostgreSQL URLs via TASK_LIVE_PG_TRANSFER_SOURCE_URL and TASK_LIVE_PG_TRANSFER_TARGET_URL"]
async fn live_postgres_transfer_preserves_data_and_schema_objects() {
    let source_url = std::env::var("TASK_LIVE_PG_TRANSFER_SOURCE_URL").expect("TASK_LIVE_PG_TRANSFER_SOURCE_URL");
    let target_url = std::env::var("TASK_LIVE_PG_TRANSFER_TARGET_URL").unwrap_or_else(|_| source_url.clone());
    ...
    let source_schema = format!("task_src_{}", &suffix[..8]);
    let target_schema = format!("task_dst_{}", &suffix[..8]);
    ...
    let dir = std::env::temp_dir().join(format!("task-live-transfer-{}", uuid::Uuid::new_v4()));
