...
#[tokio::test]
#[ignore = "requires TASK_TEST_POSTGRES_GEOMETRY_URL pointing at a PostGIS database such as local-pg/test_geometry"]
async fn live_postgres_geometry_values_render_as_text_in_query_results() {
    let url = std::env::var("TASK_TEST_POSTGRES_GEOMETRY_URL").expect("TASK_TEST_POSTGRES_GEOMETRY_URL");
    let pool = task_core::db::postgres::connect(&url, Duration::from_secs(5)).await.expect("connect");
    let result = task_core::db::postgres::execute_query(
        &pool,
