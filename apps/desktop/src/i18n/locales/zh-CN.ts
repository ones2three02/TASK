export default {
  ...
  updates: {
    ...
    platformUnavailable: "当前最新版本没有适配这台设备的安装包，请等待完整发布后再更新，或打开下载页手动查看。",
    portableManualUpdate: "便携版不能使用应用内安装器更新。请从 release 下载便携版 ZIP，并解压覆盖当前 TASK 目录，以保留 portable.task 和 data。",
  ...
  connection: {
    ...
    jdbcDocs: "查看 JDBC 文档",
    jdbcPluginHint: "先安装 TASK JDBC 插件，再导入数据库厂商提供的 JDBC 驱动 JAR。",
  ...
  welcome: {
    title: "数据库工作台",
    subtitle: "选择左侧连接开始浏览结构，或直接创建查询标签页。TASK 支持多种数据库、查询历史、数据表格编辑和 AI SQL 助手。",
    connections: "连接",
    connected: "已连接",
    databaseTypes: "数据库类型",
  ...
  configExport: {
    ...
    cryptoUnavailable: "当前浏览器环境不支持加密配置导入，请使用 HTTPS、localhost 或 TASK 桌面版。",
  ...
  ai: {
    ...
    apply: "应用到编辑器",
    clear: "清空对话",
    welcome: "我是您的 TASK 项目 AI 助手，有什么可以帮您？",
    newChat: "新对话",
    thinking: "思考中...",
    reasoningProcess: "思考过程",
    stopGenerating: "停止生成",
    tableMentionEmpty: "没有匹配的项目",
    tableMentionPlaceholderHint: "（AI 将结合当前项目的目标和行动进行规划）",
    tableMentionTypes: {
      table: "表",
      view: "视图",
      systemTable: "系统表",
    ...
    placeholders: {
      generate: "描述你想规划的项目的目标或任务，例如：为一个新功能编写设计和接口规范",
      explain: "输入你想分析的具体行动或任务内容，让 AI 评估其关键路径和难点",
      optimize: "输入当前的行动步骤，让 AI 优化并细化行动的执行路径",
      fix: "输入目标或任务描述，让 AI 生成详细的成功验收标准 (Acceptance Criteria)",
      convert: "输入你要草拟的文档或总结，让 AI 整理为结构清晰的 Markdown 文档",
      sampleData: "输入你想脑暴的关于项目的任何想法、计划或挑战",
    },
  },
  contextMenu: {
    openConnection: "打开连接",
  ...
  settings: {
    ...
    iconThemeDefault: "默认",
    iconThemeDefaultDescription: "使用彩色 TASK 应用图标。",
    iconThemeBlack: "黑色",
    iconThemeBlackDescription: "窗口、托盘和 Logo 都使用黑色 TASK 标识。",
    showTrayIcon: "显示系统托盘/菜单栏图标",
    showTrayIconDescription: "关闭后不显示图标，但关闭窗口仍会像之前一样隐藏到后台。",
    updateNotificationsEnabled: "启用更新提醒",
    updateNotificationsEnabledDescription: "关闭后，TASK 不会自动检查应用和驱动更新，也不会显示更新红点；仍可手动检查更新。",
    ...
    syncWebDavDescription: "将 TASK 快照上传到兼容 WebDAV 的存储服务，或从远端恢复。",
    ...
    syncRemotePathDescription: "上传时 TASK 会自动创建缺失的父目录。",
    syncSecretNotice: "默认只同步连接信息和设置，不会同步数据库密码、SSH 密码、代理密码、连接串或 AI API Key。",
    syncSecrets: "同步加密后的敏感信息",
    syncSecretsDescription: "开启后，TASK 会先加密数据库密码、SSH 密码和 AI API Key，再上传到 WebDAV。",
    syncSecretsPassphrase: "同步密码",
    syncSecretsPassphraseDescription: "这个密码只用于加密和恢复敏感信息，TASK 不会保存；换设备恢复时需要再次输入。",
    syncTest: "测试",
    syncUpload: "上传",
    syncDownload: "下载",
    ...
    syncDownloadConfirm: "要下载并应用远端 TASK 快照吗？本地元信息和保存的 SQL 会被替换。",
    apply: "应用",
    applyAndClose: "应用并关闭",
    reset: "重置",
    resetDefaults: "恢复默认",
    ...
    mcpTitle: "MCP Server",
    mcpDescription: "检查 Claude Code、Cursor 等编程助手使用的 TASK MCP Server 安装与版本状态。",
    mcpChecking: "检查中",
    ...
    mcpReadonlyModeDescription: "开启后会为示例配置附加 TASK_MCP_ALLOW_WRITES=0，MCP 会话只允许查询。",
    mcpAllowDangerous: "允许危险 SQL",
    mcpAllowDangerousDescription: "开启后会为示例配置附加 TASK_MCP_ALLOW_DANGEROUS_SQL=1，允许 DROP、TRUNCATE、ALTER 等语句。",
    mcpDetectionTiming: "打开此页时会自动检测，点击重新检查可刷新。",
    mcpNpmBoundary: "TASK 只检测和提示 MCP 状态；安装与升级仍由 npm 完成。",
    mcpRefresh: "重新检查",
    mcpGuide: "MCP 指南",
  ...
  driverStore: {
    ...
    javaRuntimeManaged: "TASK 托管 JRE",
