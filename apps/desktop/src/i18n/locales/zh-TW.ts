export default {
  ...
  updates: {
    ...
    availableMessage: "TASK {latest} 已發布，目前版本為 {current}。",
    upToDate: "TASK 已是最新版本 ({version})。",
    failed: "檢查更新失敗：{error}",
    rateLimited: "GitHub 更新檢查暫時觸發頻率限制。你仍然可以開啟下載頁手動檢視。",
    openRelease: "開啟下載頁",
    downloadAndInstall: "下載並安裝",
    portableManualUpdate: "可攜版無法使用應用程式內建安裝器更新。請從發行頁下載可攜版 ZIP，並解壓縮覆蓋目前的 TASK 目錄，以保留 portable.task 和 data。",
  ...
  connection: {
    ...
    jdbcDocs: "檢視 JDBC 文件",
    jdbcPluginHint: "先安裝 TASK JDBC 外掛程式，再匯入資料庫廠商提供的 JDBC 驅動程式 JAR。",
  ...
  grid: {
    ...
    queryEditUnsupported: {
      ...
      "metadata-unavailable": "TASK 無法載入資料表 metadata，因此已停用結果編輯。",
    },
  ...
  configExport: {
    ...
    cryptoUnavailable: "目前瀏覽器環境不支援加密設定匯入，請使用 HTTPS、localhost 或 TASK 桌面版。",
  ...
  settings: {
    ...
    iconThemeDefault: "預設",
    iconThemeDefaultDescription: "使用彩色 TASK 應用程式圖示。",
    iconThemeBlack: "黑色",
    iconThemeBlackDescription: "視窗、系統匣和 Logo 都使用黑色 TASK 標誌。",
    showTrayIcon: "顯示系統匣/選單欄圖示",
    showTrayIconDescription: "關閉後不顯示圖示，但關閉視窗仍會像之前一樣隱藏到後台。",
    updateNotificationsEnabled: "啟用更新提醒",
    updateNotificationsEnabledDescription: "關閉後，TASK 不會自動檢查應用程式和驅動程式更新，也不會顯示更新紅點；仍可手動檢查更新。",
    ...
    syncWebDavDescription: "上傳 TASK 快照到相容 WebDAV 的儲存服務，或從該服務還原快照。",
    ...
    syncRemotePathDescription: "上傳時 TASK 會自動建立缺失的父目錄。",
    syncSecretNotice: "預設只同步連線詳細資料和設定。資料庫密碼、SSH 密碼、代理伺服器密碼、連線字串和 AI API Key 會保留在本機。",
    syncSecrets: "同步加密後的敏感資訊",
    syncSecretsDescription: "開啟後，TASK 會先加密資料庫密碼、SSH 密碼和 AI API Key，再上傳到 WebDAV。",
    syncSecretsPassphrase: "同步密碼",
    syncSecretsPassphraseDescription: "此密碼只用於加密和還原敏感資料。TASK 不會儲存；每台裝置都需要再次輸入。",
    syncTest: "測試",
    syncUpload: "上傳",
    syncDownload: "下載",
    ...
    syncDownloadConfirm: "要下載並套用遠端 TASK 快照嗎？本機 metadata 和已儲存的 SQL 會被取代。",
    apply: "套用",
    applyAndClose: "套用並關閉",
    reset: "重設",
    resetDefaults: "還原預設",
  ...
  driverStore: {
    ...
    javaRuntimeManaged: "TASK 託管 JRE",
