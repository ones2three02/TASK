...
export const TASK_ROWID_COLUMN = "__TASK_ROWID";
export const TASK_NEO4J_ELEMENT_ID_COLUMN = "__TASK_ELEMENT_ID";
export const TASK_TDENGINE_TBNAME_COLUMN = "tbname";
...
export function editablePrimaryKeys(databaseType: DatabaseType | undefined, columns: ColumnInfo[]): string[] {
  ...
  if (databaseType === "tdengine" && primaryKeys.length > 0) return [TASK_TDENGINE_TBNAME_COLUMN, ...primaryKeys];
  const syntheticKey = getDatabaseCapability(databaseType).syntheticKey;
  if (syntheticKey === "oracle-rowid" && primaryKeys.length === 0) return [TASK_ROWID_COLUMN];
  if (syntheticKey === "neo4j-element-id" && primaryKeys.length === 0) return [TASK_NEO4J_ELEMENT_ID_COLUMN];
  return primaryKeys;
}
...
export function canEditExistingTableRows(databaseType: DatabaseType | undefined, hiveTableTransactional?: boolean, primaryKeys?: string[]): boolean {
  ...
  if (databaseType === "tdengine" && !primaryKeys?.some((key) => key.toLowerCase() === TASK_TDENGINE_TBNAME_COLUMN)) return false;
...
export function usesSyntheticRowIdKey(databaseType: DatabaseType | undefined, primaryKeys: string[]): boolean {
  return primaryKeys.length === 1 && ((databaseType === "oracle" && primaryKeys[0].toUpperCase() === TASK_ROWID_COLUMN) || (databaseType === "neo4j" && primaryKeys[0] === TASK_NEO4J_ELEMENT_ID_COLUMN));
}

export function isHiddenGridColumn(databaseType: DatabaseType | undefined, column: string, primaryKeys: string[]): boolean {
  if (databaseType === "neo4j" && column === TASK_NEO4J_ELEMENT_ID_COLUMN) return true;
  return usesSyntheticRowIdKey(databaseType, primaryKeys) && column.toUpperCase() === TASK_ROWID_COLUMN;
}
...
export function isTdengineExistingRowReadonlyColumn(databaseType: DatabaseType | undefined, column: string, columns: ColumnInfo[]): boolean {
  ...
  if (column.toLowerCase() === TASK_TDENGINE_TBNAME_COLUMN) return true;
